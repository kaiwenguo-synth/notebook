...
youtube_clips AS (
    SELECT
        ...
        transcoded_video_s3_path AS video_s3_path,
...
SELECT 
    ...
    SUM(c_duration_s) AS total_seconds,
    SUM(c_duration_s) / 3600.0 AS total_hours,
    AVG(c_duration_s) AS avg_duration_seconds,
    MIN(c_duration_s) AS min_duration_seconds,
    MAX(c_duration_s) AS max_duration_seconds
FROM all_clips
WHERE dataset_name IS NOT NULL
...
if len(results) > 0:
    ...
    total_videos = results["total_videos"].sum()
    total_clips = results["total_clips"].sum()
    total_hours = results["total_hours"].sum()
    ...
    print(f"   Total time: {total_hours:.1f} hours ({total_hours * 60:.0f} minutes)")
    ...
    for row in results.iter_rows(named=True):
        ...
        print(f"   {dataset}: {videos:,} videos, {clips:,} clips, {hours:.2f} hours")
